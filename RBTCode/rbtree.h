...
private:
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        if(node == nullptr){
        	...
        	size_t w;
        	w = width(node->left, level - 1) + width(node->right, level - 1);
        	return w;
        }
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        if(node == nullptr){
        	...
        	size_t l;
        	l = level + sum_levels(node->left, level + 1) + sum_levels(node->right, level + 1);
        	return l;
        }
    }
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        if(node == nullptr){
        	...
        	size_t n;
        	n = sum_null_levels(node->left, level + 1) + sum_null_levels(node->right, level + 1);
        	return n;
        }
    }
};
